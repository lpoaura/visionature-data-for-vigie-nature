...
/*DROP TABLE IF EXISTS pr_vigienature.cor_taxon_referentiels;*/
        ...
        COMMENT ON COLUMN pr_vigienature.cor_taxon_referentiels.taxref_cd_nom IS 'CD_NOM TaxRef';
        ...
        CREATE INDEX cor_taxon_referentiels_idx_cd_nom ON pr_vigienature.cor_taxon_referentiels (taxref_cd_nom);
...
--DROP TABLE pr_vigienature.dict_distance_code;
        ...
        COMMENT ON COLUMN pr_vigienature.t_nomenclature.code IS 'Code VisioNature';
        ...
        CREATE INDEX idx_nomenclature_code ON pr_vigienature.t_nomenclature (code);
        CREATE INDEX idx_nomenclature_code_vn ON pr_vigienature.t_nomenclature (code_vn);
...
-- ;
        ...
        (
            ...
            time                    TIME,                                              -- Heure du relevé
            observer                VARCHAR(100),                                      -- Observateur
            ...
            site                    BOOLEAN,                                           -- Est un STOC SITE
            bdd_source              VARCHAR(50),                                       -- Base de donnée source
            bdd_source_id           INTEGER,                                           -- identifiant dans la base de donnée source
            bdd_source_id_universal VARCHAR(250) UNIQUE,                               -- Identifiant unique universel dans la base de donnée source
            type_releve             VARCHAR(20),                                       -- Type de relevé (point vs transect)
            nom_protocole           VARCHAR(20),                                       -- Code du protocole (STOC EPS, STOC SITE, SHOC, etc.)
            geom_point              GEOMETRY(point, 2154),                             -- Geométrie de type point
            geom_transect           GEOMETRY(linestring, 2154),                        -- Géométrie de type transect
            source_data             JSONB,                                             -- Donnée source au format JSON
            update_ts
                                    TIMESTAMP,
...
/* Commentaire des champs de la table pr_vigienature.t_releve */
        ...
        COMMENT ON COLUMN pr_vigienature.t_releve.date IS 'Date du relevé';
        COMMENT ON COLUMN pr_vigienature.t_releve.time IS 'Heure du relevé';
        COMMENT ON COLUMN pr_vigienature.t_releve.observer IS 'Observateur';
...
/* Index sur les colonnes carre_numnat, date et point_num */
        ...
        CREATE UNIQUE INDEX ON pr_vigienature.t_releve (carre_numnat, date, point_num, bdd_source_id_universal);
        CREATE INDEX ON pr_vigienature.t_releve (carre_numnat, date, point_num);
        CREATE INDEX ON pr_vigienature.t_releve (bdd_source);
        CREATE INDEX ON pr_vigienature.t_releve (bdd_source_id);
...
-- ;
        ...
        (
            ...
            details                 JSONB,
            bdd_source              VARCHAR(50),
            bdd_source_id           INTEGER,
            bdd_source_id_universal VARCHAR(50),
            source_data             JSONB,
            update_ts               TIMESTAMP
        );
...
/**/
        ...
        COMMENT ON COLUMN pr_vigienature.t_observation.id IS 'Identifiant unique universel';
        ...
        COMMENT ON COLUMN pr_vigienature.t_observation.bdd_source IS 'Base de donnée source';
...
/* INFO: Index de la table pr_vigienature.t_observation */
        ...
        CREATE INDEX ON pr_vigienature.t_observation (bdd_source);
