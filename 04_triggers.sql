...
DROP FUNCTION IF EXISTS pr_stoc.get_euring_code_from_vn_id_species
;

CREATE OR REPLACE FUNCTION pr_stoc.get_euring_code_from_vn_id_species(id_species INT, OUT result VARCHAR(20))
    RETURNS VARCHAR(20) AS
$$
BEGIN
    EXECUTE format(
            'SELECT match_taxa_repo.euring_code from pr_stoc.match_taxa_repo where vn_id_species = $1 and ref_tax is true limit 1')
        INTO result
        USING id_species;
END ;
$$
...
DROP FUNCTION IF EXISTS pr_stoc.get_survey_id_from_id_form_uid
;

CREATE OR REPLACE FUNCTION pr_stoc.get_survey_id_from_id_form_uid(id_form_universal VARCHAR(50), OUT result INT)
    RETURNS INT AS
$$
BEGIN
    EXECUTE format(
            'SELECT t_survey.id from pr_stoc.t_survey where db_source_id_universal = $1 limit 1')
        INTO result
...
/*
select pr_stoc.get_euring_code_from_vn_id_species(518);
*/
...
BEGIN
    ...
    DELETE
        FROM
            pr_stoc.t_survey
        WHERE
            db_source_id_universal = old.item ->> 'id_form_universal';
    IF NOT found
    THEN
        RETURN NULL;
    END IF;
...
CREATE OR REPLACE FUNCTION pr_stoc.upsert_survey() RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
DECLARE
    ...
    the_db_source_id_universal VARCHAR(20);
    the_db_source              VARCHAR(20);
...
BEGIN
    ...
    the_altitude = pr_stoc.get_altitude_from_dem(st_transform(
            ...
            2154));
    the_cloud = pr_stoc.get_code_point_values_from_vn_code('code'::TEXT, new.item #>> '{protocol, stoc_cloud}');
    the_rain = pr_stoc.get_code_point_values_from_vn_code('code'::TEXT, new.item #>> '{protocol, stoc_rain}');
    the_wind = pr_stoc.get_code_point_values_from_vn_code('code'::TEXT, new.item #>> '{protocol, stoc_wind}');
    the_visibility =
    ...
    the_db_source_id_universal = new.item ->> 'id_form_universal';
    the_db_source = new.site;
    ...
    THEN
        ...
        SET
          ...
          , db_source              = the_db_source
          , db_source_id_universal = the_db_source_id_universal
          , survey_geotype         = the_survey_geotype
            WHERE
--                 (t_survey.carre_numnat, t_survey.date, t_survey.point_num, t_survey.db_source_id_universal) =
--                 (the_carre_numnat, the_date, the_point_num, the_db_source_id_universal);
t_survey.db_source_id_universal = the_db_source_id_universal;
        IF NOT found
        THEN
            INSERT INTO
                pr_stoc.t_survey ( date
                                 , time
                                 , observer
                                 , carre_numnat
                                 , point_num
        ...
        THEN
            INSERT INTO
                ...
                VALUES
                    ...
                    , the_db_source_id_universal
                    , the_survey_geotype)
            ON CONFLICT (db_source_id_universal) DO UPDATE SET
                                                               date                   = the_date
                                                             , time                   = the_time
                                                             , observer               = the_observer
                                                             , carre_numnat           = the_carre_numnat
...
EXECUTE PROCEDURE pr_stoc.upsert_survey()
;
...
BEGIN
    is_stoc_eps = new.id_form_universal IN
                  (SELECT db_source_id_universal FROM pr_stoc.t_survey WHERE survey_geotype IS NULL);
    IF is_stoc_eps
    THEN
        UPDATE pr_stoc.t_survey
        SET
            survey_geotype = CASE
                ...
                END
            WHERE
                  db_source_id_universal = new.id_form_universal
...
/* Trigger sur les observations, conditions:
   id_form_universal not null and id_form_universal in (select id_form_universal from pr_stoc.t_survey) */
...
BEGIN
    DELETE
        ...
        WHERE
            (db_source, db_source_id) = (old.site, old.id);
    IF NOT found
    THEN
        RETURN NULL;
    END IF;
...
DECLARE
    ...
    the_stoc_obs      RECORD;
BEGIN
    is_stoc_eps = new.id_form_universal IN (SELECT db_source_id_universal FROM pr_stoc.t_survey);
    IF is_stoc_eps
    THEN
        the_survey_id = pr_stoc.get_survey_id_from_id_form_uid(new.id_form_universal);
        the_euring_codesp = pr_stoc.get_euring_code_from_vn_id_species(cast(new.item #>> '{species, @id}' AS INT));
        IF (tg_op = 'UPDATE')
        THEN
--         RAISE NOTICE 'DELETE DATA FROM RELEVE %', pr_stoc.get_survey_id_from_id_form_uid(new.id_form_universal);
            DELETE
                FROM
                    pr_stoc.t_observations
                WHERE
        ...
        END IF;
        RAISE DEBUG 'INSERT DATA FROM RELEVE %', pr_stoc.get_survey_id_from_id_form_uid(new.id_form_universal);
        INSERT
            INTO
            pr_stoc.t_observations ( survey_id
                                   , euring_codesp
                                   , vn_is_species
                                   , count
                                   , distance
        ...
        SELECT
          ...
          , obs.site                                                           AS db_source
          , obs.id                                                             AS db_source_id
